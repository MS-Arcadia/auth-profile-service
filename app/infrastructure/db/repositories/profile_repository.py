    ...
    async def add_owned_game(self, owned_game: OwnedGame) -> None:
        self._session.add(
            OwnedGameModel(
                ...
                hidden=owned_game.hidden,
                acquired_at=owned_game.acquired_at,
            )
        )
        await self._session.commit()
    ...
    async def add_owned_item(self, owned_item: OwnedItem) -> None:
        self._session.add(
            OwnedItemModel(
                ...
                game_id=owned_item.game_id,
                acquired_at=owned_item.acquired_at,
            )
        )
        await self._session.commit()
    ...
    async def upsert_top_post(self, top_post: TopPost) -> None:
        existing = (
            await self._session.execute(
                select(TopPostModel).where(
                    TopPostModel.user_id == top_post.user_id,
                    TopPostModel.post_id == top_post.post_id,
                )
            )
        ).scalar_one_or_none()
        ...
        if existing:
            ...
            self._session.add(
                TopPostModel(
                    id=top_post.id,
                    user_id=top_post.user_id,
                    post_id=top_post.post_id,
                    feedback_score=top_post.feedback_score,
                    rank=0,
                )
            )
        await self._session.commit()
        ...
        all_posts = (
            (
                await self._session.execute(
                    select(TopPostModel)
                    .where(TopPostModel.user_id == top_post.user_id)
