...
from app.application.ports.auth_ports import UserRepositoryPort, RoleRequestRepositoryPort
from app.domain.auth.user import User
from app.domain.auth.role_request import RoleRequest
from app.domain.auth.enums import Role, UserState, RoleRequestStatus
...
def _to_domain_user(row: UserModel) -> User:
    return User(
        ...
        display_name=row.display_name, role=Role(row.role), state=UserState(row.state),
...
def _to_domain_role_request(row: RoleRequestModel) -> RoleRequest:
    return RoleRequest(
        id=row.id, user_id=row.user_id, requested_role=Role(row.requested_role),
        status=RoleRequestStatus(row.status), decision_note=row.decision_note,
